...
export class TaskStatusService {
    ...
    private isTransitionAllowed(
        ...
        if (stageType === WorkflowStageType.COMPLETION) {
            if (!isManager) {
                return { allowed: false, reason: 'Completion stage transitions require manager permissions' };
            }
        }
        ...
        switch (fromStatus) {
            ...
            case TaskStatus.SUSPENDED:
                return toStatus === TaskStatus.IN_PROGRESS
                    ? { allowed: true }
                    : { allowed: false, reason: 'Can only resume suspended tasks to IN_PROGRESS' };
