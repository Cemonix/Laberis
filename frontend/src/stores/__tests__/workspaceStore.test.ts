...
describe('Workspace Store', () => {
  ...
  const mockLabelScheme: LabelScheme = {
    ...
    projectId: 1,
    labels: [
      { labelId: 10, name: 'Test Label', color: '#FF0000', labelSchemeId: 2, description: 'A test label' },
      { labelId: 11, name: 'Another Label', color: '#00FF00', labelSchemeId: 2, description: 'Another test label' }
    ]
  }
  ...
  describe('Initial State', () => {
    ...
    it('should have default available tools', () => {
      ...
      expect(workspaceStore.availableTools.map(tool => tool.id)).toEqual([
        ToolName.CURSOR,
        ToolName.POINT,
    ...
    it('should have timer instance', () => {
      ...
      expect(workspaceStore.timerInstance).toHaveProperty('getFormattedElapsedTime')
    })
  })
  ...
  describe('Getters', () => {
    ...
    it('should get active tool details', () => {
      expect(workspaceStore.getActiveToolDetails?.id).toBe(ToolName.CURSOR)
      expect(workspaceStore.getActiveToolDetails?.name).toBe('Cursor')

      workspaceStore.setActiveTool(ToolName.POINT)
      expect(workspaceStore.getActiveToolDetails?.id).toBe(ToolName.POINT)
    ...
    it('should get current label scheme', () => {
      ...
      expect(workspaceStore.getCurrentLabelScheme?.name).toBe('Default Scheme')

      workspaceStore.setCurrentLabelScheme(mockLabelScheme)
      expect(workspaceStore.getCurrentLabelScheme).toEqual(mockLabelScheme)
  ...
  describe('Asset Loading', () => {
    it('should load asset successfully', async () => {
      ...
      expect(workspaceStore.currentImageUrl).toMatch(/https:\/\/picsum\.photos\/800\/600\?random=/)
      expect(workspaceStore.imageNaturalDimensions).toBeNull()
    ...
    it('should handle fetch annotations error during asset loading', async () => {
      ...
      vi.mocked(fetchAnnotations).mockRejectedValue(new Error('Network error'))

      await workspaceStore.loadAsset('project-1', 'asset-1')

      expect(workspaceStore.annotations).toEqual([])
      expect(consoleError).toHaveBeenCalledWith('Failed to fetch annotations:', expect.any(Error))
  ...
  describe('Image Dimensions', () => {
    it('should set image natural dimensions', () => {
      ...
      expect(workspaceStore.imageNaturalDimensions).toEqual(mockImageDimensions)
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Set image natural dimensions:', mockImageDimensions)
    ...
    it('should set canvas display dimensions', () => {
      ...
      expect(workspaceStore.canvasDisplayDimensions).toEqual(mockImageDimensions)
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Set canvas display dimensions:', mockImageDimensions)
  ...
  describe('Annotations Management', () => {
    ...
    it('should handle annotation save failure', async () => {
      const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
      vi.mocked(saveAnnotation).mockRejectedValue(new Error('Save failed'))

      await workspaceStore.addAnnotation(mockAnnotation)

      expect(workspaceStore.annotations).toEqual([])
      expect(consoleError).toHaveBeenCalledWith('Failed to save annotation:', expect.any(Error))
      expect(window.alert).toHaveBeenCalledWith('Could not save annotation. Please try again.')
  ...
  describe('Label Management', () => {
    it('should set current label id', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Selected Label ID: 5 (Unknown)')

      workspaceStore.setCurrentLabelId(null)
      expect(workspaceStore.currentLabelId).toBeNull()
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Label selection cleared.')
    ...
    it('should set current label id with known label name', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Selected Label ID: 10 (Test Label)')
    ...
    it('should set current label scheme', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Label Scheme set:', 'Test Scheme')

      workspaceStore.setCurrentLabelScheme(null)
      expect(workspaceStore.currentLabelScheme).toBeNull()
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Label Scheme cleared.')
  ...
  describe('Task Management', () => {
    it('should set current task id', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Current Task ID set:', 123)

      workspaceStore.setCurrentTaskId(null)
      expect(workspaceStore.currentTaskId).toBeNull()
      expect(consoleSpy).toHaveBeenCalledWith('[Store] Current Task ID cleared.')
  ...
  describe('Private Methods', () => {
    ...
    it('should update elapsed time display', () => {
      vi.mocked(workspaceStore.timerInstance.getFormattedElapsedTime).mockReturnValue('01:23:45')
  ...
  describe('Default Label Scheme', () => {
    it('should have default label scheme with sample labels', () => {
      ...
      expect(workspaceStore.currentLabelScheme?.name).toBe('Default Scheme')
      expect(workspaceStore.currentLabelScheme?.labels).toHaveLength(3)
      expect(workspaceStore.currentLabelScheme?.labels.map(l => l.name)).toEqual(['Person', 'Car', 'Tree'])
    })
  })
})
